#!/usr/bin/env python3
"""
speed_by_exit_directionV3.py

Computes average speed by EXIT direction (N/E/S/W) using FastAPI endpoint:
...
- That record's speed_mps is the "exit speed" used in stats.
...
def main():
    ap = argparse.ArgumentParser(description="Average speed by EXIT direction from FastAPI endpoint.")
    ...
    # Total stats: mean speed per direction + counts
    total = (
    ...
    fig_total = px.bar(
        ...
        x="direction",
        y="avg_speed_mph",
        title="Average EXIT speed by direction (API)",
    ...
    # Timeseries: average exit speed per direction per time bin
    if interval:
        ...
        fig_ts = px.line(
            ...
            y="avg_speed_mph",
            color="direction",
            title=f"Average EXIT speed over time (bin={interval}, API)",
            labels={"time_bin": "Exit time bin", "avg_speed_mph": "Average speed (mph)"},
            hover_data={"count": True},
        )
        ...
        out_ts_json.write_text(ts_stats.to_json(orient="records", date_format="iso"), encoding="utf-8")
